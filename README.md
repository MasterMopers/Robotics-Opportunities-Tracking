...
Every listing below is pulled automatically from the source's own page (never summarized or guessed by AI) and re-checked weekly, so treat it as a starting point -- **always confirm current details on the official page before you apply**, especially the deadline. Filters skew toward opportunities realistic for one or two people: no minimum team size beyond a couple people, no "must be a registered company" or "requires a faculty advisor" grants.
...
`.github/workflows/monitor.yml` runs `monitor.py` weekly (new items -> opens a GitHub issue) and rebuilds this README monthly. See `sources.yaml` for every tracked source and the watch calendar, and `rules.yaml` for the classification and field-extraction rules -- tune both without touching code. Classification is fully deterministic (regex/keyword rules), with no AI/LLM calls anywhere in the pipeline.
